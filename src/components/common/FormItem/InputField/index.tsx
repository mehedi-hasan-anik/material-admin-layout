...
export const FormInputLabel: FC<FormInputLabels> = ({
  children,
  required,
  ...otherProps
}) => {
...
const InputField: FC<FormInputProps> = ({ name, setError, ...otherProps }) => {
  const {
    control,
    formState: { errors },
  } = useFormContext();
      ...
      render={({ field: { ref, ...field } }) => (
        <FormControl variant="standard" fullWidth>
          <BootstrapInput
            ...
            {...otherProps}
            fullWidth
            error={!!errors[name] || !!setError || otherProps?.errorstatus}
            inputRef={ref}
          />
