...
export const MultiLevel = ({ item, viewSidebar }: Props) => {
  ...
  useEffect(() => {
    const result = children?.map((ele: any) => {
      if (router.pathname.includes(`/${ele?.pathTitle?.toLowerCase()}/`)) {
        setOpen(true);
      } else if (ele?.items?.length > 0) {
        const res2 = ele.items?.map(
          (ele2: any) => ele2.path === router.pathname
        );
        return res2;
      } else {
